// ============================================================
// Arkmonía API v3 — Function Compute (nodejs20 runtime)
// index.handler
//
// Colecciones gestionables (JSON en /tmp con semilla en primer uso):
//   usuarios, tipos_proyecto, proyectos, categorias, posts,
//   construcciones, bim, testimonios, contactos, leads
//
// CRUD admin:  GET/POST /api/admin/:col   |   PUT/DELETE /api/admin/:col/:id
// Público:     /api/health, /api/contact, /api/lead, /api/register,
//              /api/savings-calculator, /api/usuarios, /api/proyectos,
//              /api/categorias, /api/posts, /api/testimonios,
//              /api/blog, /api/construction, /api/bim, /uploads/*
// Auth:        POST /api/auth/login -> token Bearer (HMAC)
//
// Env: ADMIN_USER, ADMIN_PASS, TOKEN_SECRET, API_PUBLIC_BASE (para
//      absolutizar rutas /uploads/* en respuestas públicas)
// ============================================================
...
const UPLOAD_DIR = '/tmp/arkmonia_uploads'
...
const SEED_POSTS = [
  ...
  { id: 'ventilacion-cruzada', nombre: 'Ventilación cruzada: la clave de una casa fresca sin aire acondicionado', categoria: 'Educativo', usuario: 'Guillermo Ortiz', estatus: 'Publicado',
    ...
    contenido: ['La ventilación cruzada consiste en permitir que el aire fluya a través de la vivienda mediante aberturas en fachadas opuestas.', 'Para lograrla se consideran la orientación del terreno, los vientos dominantes y la posición de ventanas y patios.', 'Bien ejecutada, puede reducir varios grados la sensación térmica sin gastar un solo watt.'], imagenes: [] },
  { id: 'materiales-termicos', nombre: 'Materiales térmicos para el clima de Riviera Maya: qué funciona', categoria: 'Educativo', usuario: 'Guillermo Ortiz', estatus: 'Publicado',
    ...
    contenido: ['En el clima de Yucatán, los materiales térmicos son la primera línea de defensa contra el calor.', 'Estos materiales retrasan la transferencia de calor hacia el interior, manteniendo la casa fresca durante el día.', 'La inversión inicial se amortiza con ahorros sostenidos en climatización a lo largo de la vida útil del edificio.'], imagenes: [] },
...
function load(col) {
  ...
  if (def.seed && def.seed.length) {
    try { fs.writeFileSync(file, JSON.stringify(def.seed)) } catch (_) {}
    return def.seed
  }
  return []
}
function save(col, items) {
  try { fs.writeFileSync(COLLECTIONS[col].file, JSON.stringify(items)) } catch (_) {}
}

// Documento único de la oferta (no es colección de registros)
function loadOferta() {
  try {
...
function normalizeImagenes(imgs) {
  ...
  for (const s of imgs) {
    ...
    const v = s.trim()
    if (!v) continue
    out.push(v.startsWith('data:') ? saveUpload(v) : v)
  }
...
function finalize(col, body) {
  const b = {}
  for (const [k, v] of Object.entries(body || {})) {
    if (k === 'imagenes') { b[k] = normalizeImagenes(v); continue }
...
exports.handler = (event, _context, callback) => {
  ...
  if (pathname === '/api/health' && method === 'GET') {
    return callback(null, { statusCode: 200, headers, body: JSON.stringify({ ok: true, service: 'arkmonia-api-v3', ts: new Date().toISOString() }) })
  }
  ...
  // Oferta (documento editable desde el panel)
  if (pathname === '/api/oferta' && method === 'GET') {
    return callback(null, { statusCode: 200, headers, body: JSON.stringify({ ok: true, oferta: loadOferta() }) })
  }

  // Listados públicos
  if (pathname === '/api/usuarios' && method === 'GET') {
    return callback(null, { statusCode: 200, headers, body: JSON.stringify({ ok: true, items: load('usuarios') }) })
  }
  if (pathname === '/api/proyectos' && method === 'GET') {
    return callback(null, { statusCode: 200, headers, body: JSON.stringify({ ok: true, items: load('proyectos').map((p) => ({ ...p, imagenes: absImgs(p.imagenes) })) }) })
  }
  if (pathname === '/api/categorias' && method === 'GET') {
    return callback(null, { statusCode: 200, headers, body: JSON.stringify({ ok: true, items: load('categorias') }) })
  }
  if (pathname === '/api/posts' && method === 'GET') {
    const items = load('posts').filter((p) => (p.estatus || 'Publicado') === 'Publicado').map((p) => ({ ...p, imagenes: absImgs(p.imagenes) }))
    return callback(null, { statusCode: 200, headers, body: JSON.stringify({ ok: true, items }) })
  }
  if (pathname === '/api/testimonios' && method === 'GET') {
    const items = load('testimonios').filter((t) => t.aprobado !== false).map((t) => ({ ...t, imagenes: absImgs(t.imagenes) }))
    return callback(null, { statusCode: 200, headers, body: JSON.stringify({ ok: true, items }) })
  }
  if (pathname === '/api/blog' && method === 'GET') {
    const items = load('posts').filter((p) => (p.estatus || 'Publicado') === 'Publicado').map((p) => ({
  ...
  if (pathname === '/api/construction' && method === 'GET') {
    const items = load('construcciones').map((c) => ({
  ...
  if (colMatch && COLLECTIONS[colMatch[1]]) {
    ...
    if (method === 'GET') {
      return callback(null, { statusCode: 200, headers, body: JSON.stringify({ ok: true, items: load(col) }) })
    }
    if (method === 'POST') {
      const body = parseBody(req)
      if (!body) return callback(null, { ...bad('JSON inválido'), headers })
      const clean = finalize(col, body)
...
// No afecta Function Compute ni añade rutas HTTP.
